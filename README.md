...
화면비는 **16:9 로 고정**합니다. 단말 해상도에 맞춰 늘리지 않고, 가장 큰 16:9 사각형을
화면 가운데에 놓은 뒤 남는 곳은 검은 띠로 둡니다. 게임 월드, UI 화면, 가상 조이스틱이
모두 같은 사각형을 기준으로 놓이므로 어느 기기에서나 같은 화면이 나옵니다.
(`render/StageBox.kt`, [docs/SCREENS.md](./docs/SCREENS.md) §7)
...
설정 화면만 예외로, 다른 화면 **위에** 덮입니다. 설정은 두 장입니다 — 방 규칙을
정하는 **게임 설정**은 로비에서 방장만, 소리 크기를 맞추는 **사운드 설정**은 메인
메뉴 오른쪽 아래에서 누구나 엽니다.
