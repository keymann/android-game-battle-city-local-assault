...
/**
 * 테스트 전용 최소 PNG 디코더 (8bit 팔레트, 비인터레이스).
 *
 ...
 *
 * 지원 범위는 이 프로젝트의 아틀라스(`tiny.png`)가 쓰는 형식으로 한정한다.
...
class IndexedPngReader private constructor(
    ...
    companion object {
        ...
        fun read(file: File): IndexedPngReader {
            ...
            var alphas: IntArray = IntArray(0)
            ...
            while (offset + 8 <= bytes.size) {
                ...
                when (type) {
                    "IHDR" -> {
                        ...
                        require(colorType == 3) { "팔레트 PNG 만 지원한다 (colorType=$colorType)" }
            ...
            val raw = inflate(idat.toByteArray(), (width + 1) * height)
            val indices = unfilter(raw, width, height)

            val pixels = IntArray(width * height) { i ->
                val paletteIndex = indices[i]
                val alpha = if (paletteIndex < alphas.size) alphas[paletteIndex] else 0xFF
                (alpha shl 24) or colors[paletteIndex]
            }
        ...
        /** 8bit 팔레트라 bytes-per-pixel 은 1 이다. */
        private fun unfilter(raw: ByteArray, width: Int, height: Int): IntArray {
            ...
            for (y in 0 until height) {
                ...
                for (x in 0 until width) {
                    ...
                    val left = if (x > 0) current[x - 1].toInt() and 0xFF else 0
                    val up = previous[x].toInt() and 0xFF
                    val upperLeft = if (x > 0) previous[x - 1].toInt() and 0xFF else 0
                    ...
                    current[x] = restored.toByte()
                    result[y * width + x] = restored
                }
